...
def newTask():
    task = my_entry.get()
    if task != "":
        lb.insert(END, task)#A variavel é lb atribuída à caixa de listagem.
...
'''
Descricao: O usuario ira clicar o item em logo depois em um botão com o nome 'Delete Task'
...
Neste caso, se o usuário clica no addTask_btn botão, em seguida, a função newtask 
é chamado e quando o usuário clica no delTask_btn Botão então função delTask é chamado e 
o mesmo para o delall.
...
fill=BOTH  isso irá preencher o espaço em branco em ambas as direções que são xe y.
'''
